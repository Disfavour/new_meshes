...
void recursion_core(const vector<rectangle>& rectangles, const size_t& unit_width,
    const size_t& k, const size_t& height, const vector<forward_list<interval>>& rows, const vector<packed_rectangle>& packed_rectangles,
        ...
        for (const auto& current_interval : rows[i]) {
            ...
            for (size_t x_left = current_interval.left; x_left <= current_interval.right - item.width; x_left++) {
                ...
                if (is_last_item) {
                    ...
                    vector<packed_rectangle> new_packed_rectangles = packed_rectangles;
                    new_packed_rectangles.push_back({x_left, x_left + item.width, i, i + item.height});

                    recursion_core(rectangles, unit_width,
                        k+1, new_height, new_rows, new_packed_rectangles,
