...
vector<packed_rectangle> solve_problem(const size_t& unit_width, const size_t& items_number, const vector<size_t>& widths, const vector<size_t>& heights) {
    ...
    for (const auto& rectangle : rectangles) {
        packed_rectangle packed_item{0, 0, 0, 0};

        size_t x_max = skyline[0].x + rectangle.width;
        size_t y_max = skyline[0].y;
        for (size_t j = 1; j < skyline.size(); j++)
        {
            if (skyline[j].x > x_max)
        ...
        for (size_t i = 1; i < skyline.size(); i++)
        {
            x_max = skyline[i].x + rectangle.width;
            if (x_max > unit_width)
                break;
        ...
        packed_item.x_max = packed_item.x_min + rectangle.width;
        packed_item.y_max = packed_item.y_min + rectangle.height;
        ...
        for (const auto& p : old_skyline) {
            ...
            if (p.x == packed_item.x_min && packed_item.y_max != previous_height) {
                skyline.push_back({p.x, packed_item.y_max});
            }
            if (p.x == packed_item.x_max) {
                skyline.push_back(p);
