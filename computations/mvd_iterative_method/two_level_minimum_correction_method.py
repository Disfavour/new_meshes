...
# - div (k * grad u) + c*u = f
def calculate(quadrangle_mesh_name, k, max_iter, info=False, plot=False):
    ...
    rs = [np.linalg.norm(np.concatenate((r_D, r_V)))]

    for i in range(max_iter):
        w_D = A_DD_inv @ r_D
        w_V = A_VV_inv @ r_V

        a = A_DD @ w_D @ w_D + A_DV @ w_V @ w_D + A_VD @ w_D @ w_V + A_VV @ w_V @ w_V
        b = w_D @ (A_DD @ w_D) + w_D @ (A_DV @ w_V) + A_DD_inv @ A_DV @ w_V @ (A_DD @ w_D) + A_DD_inv @ A_DV @ w_V @ (A_DV @ w_V) \
            + A_VV_inv @ A_VD @ w_D @ (A_VD @ w_D) + A_VV_inv @ A_VD @ w_D @ (A_VV @ w_V) + w_V @ (A_VD @ w_D) + w_V @ (A_VV @ w_V)
        tau = a / b
        ...
        rs.append(np.linalg.norm(np.concatenate((r_D, r_V))))
...
if __name__ == '__main__':
    ...
    res = calculate('meshes/rectangle/rectangle_9_quadrangle', k, 1000, info=True, plot=False)
    #print(res)
