...
def calculate(quadrangle_mesh_name, k, max_iter, tau=None, r_rel_min=-1, info=False, plot=False):
    ...
    mask_DD = (row < node_groups[0]) & (col < node_groups[0])
    mask_VV = (row >= node_groups[0]) & (col >= node_groups[0])
    row_B = np.concatenate([row[mask_DD], row[mask_VV]])
    col_B = np.concatenate([col[mask_DD], col[mask_VV]])
    data_B = np.concatenate([data[mask_DD], data[mask_VV]])
    B = coo_array((data_B, (row_B, col_B)), shape=A.shape).tocsr()
    ...
    # 0
    r_abs = [np.linalg.norm(f - A@y)]
    r_rel = [r_abs[-1] / r_abs[0]]
    ...
    y = spsolve(B, (B - tau*A)@yk + tau*f)

    r_abs.append(np.linalg.norm(f - A@y))
    r_rel.append(r_abs[-1] / r_abs[0])
    ...
    for i in range(2, max_iter + 1):
        ...
        y = spsolve(B, a*(B - tau*A)@yk + (1 - a)*B@yk1 + a*tau*f)
...
if __name__ == '__main__':
    ...
    res = calculate('meshes/rectangle/rectangle_11_quadrangle', k, 1000, 1, 1e-6, info=True, plot=False)
    #print(res)
