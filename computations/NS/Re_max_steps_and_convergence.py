...
def solve(n, p, fname):
    ...
    while Re_global < 2e4:
        ...
        errs_local = []
        while True:
            Re.value = min(Re_local + step, 2e4)
            un.x.array[:] = un_base.x.array

            if domain.comm.rank == 0:
                print(f'Re_global {Re_global} Re_local {Re_local} Re.value {Re.value}')
            ...
            #domain.comm.Barrier()
            Re_local = Re.value + 0 # тут присвоение как ссылка, а такое не надо
            errs_local = errs_local_iter
            u_local.x.array[:] = u.x.array

            if domain.comm.rank == 0:
                print(f'Set Re_local {Re_local}')
            
            if Re_local == 2e4:
                break
        
        if len(errs_local) < 20:
            raise Exception('doesnt convege')

        # if domain.comm.rank == 0:
        #     print(f'Re_local {Re_local}')
        
        Re_global = Re_local
        un_base.x.array[:] = u_local.x.array
        
        Res.append(Re_global)
        errs.append(errs_local)
...
if __name__ == '__main__':
    ...
    m = 512
    p = 2
    solve(m, p, f'data/NS/steps_m{m}_p{p}.npz')
