...
def solve(n, p):
    ...
    for Re_current in best_steps:
        ...
        while True:
            ...
            err_local = dolfinx.fem.assemble_scalar(dolfinx.fem.form(dot(w - wn, w - wn)*dx))
            err = np.sqrt(domain.comm.allreduce(err_local, op=MPI.SUM))

            if domain.comm.rank == 0:
                print(f'err {err} iter_time {time() - iter_time}')
        ...
        if np.isclose(Re.value, 1000) or np.isclose(Re.value, Re_max):
            ...
            with XDMFFile(domain.comm, f'data/NS/Re{int(Re.value)}_n{m}_p{p}.xdmf', "w") as xdmf:
            ...
            if domain.comm.rank == 0:
                np.savez(f'data/NS/extreme_values_Re{int(Re.value)}_n{m}_p{p}.npz',
