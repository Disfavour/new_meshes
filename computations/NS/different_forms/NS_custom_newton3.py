...
def calculate_residual(psi, w, psin, wn, Re, degree_raise=3):
    ...
    # a = r * rt * dx
    # L = Re*(-(dif_psi * dif_w.dx(1)).dx(0) + (dif_psi * dif_w.dx(0)).dx(1)) * rt * dx
    ...
    # integral = dolfinx.fem.form(w.dx(1).dx(1) * ufl.dx)
    # integral = dolfinx.fem.assemble_scalar(integral)
    # integral = mesh.comm.allreduce(integral, op=MPI.SUM)
    # print(integral)

    residual = dolfinx.fem.Function(W)
    residual.interpolate(dolfinx.fem.Expression(
        Re*(
            -(dif_psi * dif_w.dx(1)).dx(0)
            + (dif_psi * dif_w.dx(0)).dx(1)
        ),
...
def solve(n, p, Re, fname):
    ...
    F = dot(grad(psi), grad(r))*dx - w*r*dx - r*ds(1) \
        + dot(grad(w), grad(s))*dx + Re*(
            s.dx(0)*psi*wn.dx(1) + s.dx(0)*psin*w.dx(1) - s.dx(0)*psin*wn.dx(1)
            - (s.dx(1)*psi*wn.dx(0) + s.dx(1)*psin*w.dx(0) - s.dx(1)*psin*wn.dx(0))
        )*dx(domain)

    # F = dot(grad(psi), grad(r))*dx - w*r*dx - r*ds(1) \
    #     + dot(grad(w), grad(s))*dx + Re*(
    #         -((psi * wn.dx(1)).dx(0) + (psin * w.dx(1)).dx(0) - (psin * wn.dx(1)).dx(0))
    #         + (psi * wn.dx(0)).dx(1) + (psin * w.dx(0)).dx(1) - (psin * wn.dx(0)).dx(1)
    #     )*s*dx(domain)
...
if __name__ == '__main__':
    ...
    for m in ms:
            for Re in Res:
                solve(m, p, Re, f'data/NS/convergence3_m{m}_p{p}_Re{Re}.npz')
