...
def solve(mesh_fname, Re, k, sigma, fname, p=2, max_iter=500):
    ...
    F = dot(grad(psi), grad(r))*dx - w*r*dx - r*ds(1) \
        + dot(grad(w), grad(s))*dx + Re*(
            k*(psi.dx(1)*wn.dx(0) - psi.dx(0)*wn.dx(1))
            + (1-k)*(psin.dx(1)*w.dx(0) - psin.dx(0)*w.dx(1))
        )*s*dx(domain)
...
if __name__ == '__main__':
    mesh_fname = 'meshes/NS/ellipse_3.msh'
    Re = 10000
    kappa = 0.5
    ...
    for params in ((mesh_fname, Re, kappa, sigma, f'{prefix}_k{kappa}_s{sigma}', degree, max_iter) for sigma in sigmas):
        solve(*params)
