...
def read_mesh_from_file(mesh_fname):
    ...
    return nodes, cells, cell_edges, edges, edge_nodes, g, triangle_mean_area
...
def solve(mesh_fname, f, ue, center, K):
    """Градиент - функция по 3 точкам (не интегральное определение)"""
    nodes, cells, cell_edges, edges, edge_nodes, g, triangle_mean_area = read_mesh_from_file(mesh_fname)
    ...
    # середины ребер
    # x1, y1 = (cell_coords[:, 1] - cell_coords[:, 0]).T
    # x2, y2 = (cell_coords[:, 2] - cell_coords[:, 1]).T
    # x3, y3 = (cell_coords[:, 0] - cell_coords[:, 2]).T
    ...
    #return edges.size, np.sqrt(triangle_mean_area), np.sqrt(areas.mean()), L2, Lmax
