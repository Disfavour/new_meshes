...
def plot_results(u_numeric, u_exact, L_inf, triangle_node_tags, node_coords):
    triangle_node_tags -= 1
    print(triangle_node_tags)

    triangulation = tri.Triangulation(node_coords[:, 0], node_coords[:, 1], triangle_node_tags)
    ...
    fig.suptitle(f'$L_\infty$ = {L_inf}')

    # Первый tripcolor — численное решение
    tpc0 = axes[0].tripcolor(triangulation, u_numeric, shading='gouraud', vmin=vmin, vmax=vmax)
    axes[0].set_title("Численное решение")
    axes[0].axis('scaled')

    # Второй tripcolor — точное решение
    tpc1 = axes[1].tripcolor(triangulation, u_exact, shading='gouraud', vmin=vmin, vmax=vmax)
    axes[1].set_title("Точное решение")
    axes[1].axis('scaled')

    # Один общий colorbar
    fig.colorbar(tpc1, ax=axes, orientation='vertical', label='u')
...
def compute_area(points):
    """
    Вычисляет площадь многоугольника по вершинам, заданным в порядке обхода (2D).
    
    :param points: np.ndarray с формой (N, 2), где N — количество вершин
...
# div (k * grad u) = 0
# u = exp(x*y)
def calculate(quadrangle_mesh_name, k, u, f):
    gmsh.initialize()
    ...
    for node_D_inner, cell_nodes in zip(range(1, node_groups[0] + one), cells):
        ...
        area = compute_area(np.array([node_coords[node - one][:2] for node in cell_nodes]))
        div = div_sum / area

        #div = div_sum / cell_boundary_lenght
...
if __name__ == '__main__':
    ...
    f = lambda x, y: np.exp(x*y)*(k[0][0]*y**2 + (k[0][1] + k[1][0])*x*y + k[1][1]*x**2)
    calculate('meshes/rectangle/rectangle_0_quadrangle', k, u, f)
