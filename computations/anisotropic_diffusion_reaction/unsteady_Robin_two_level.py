...
def solve(k_f, r_f, f_f, c_f, bc_f, u_f, q_f, ts, sigma, quadrangle_mesh_fname, info=False):
    ...
    c3 = c_f(*nodes[quadrangles[boundary, 3]].T, t)
    bc3 = bc_f(*nodes[quadrangles[boundary, 3]].T, *eV[boundary].T, t)
    matrix_g = utility_mvd.assemble_matrix_g(k, lD, lV, c3, bc3, inner, boundary)
    ...
    def collect_data(i):
        ...
        g = utility_mvd.compute_global_g(B, matrix_g[:, :, :4], matrix_g[:, :, 4], y, quadrangles, boundary)
