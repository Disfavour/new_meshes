...
def solve(k, r, f_f, c, bc_f, u_f, q_f, ts, sigma, quadrangle_mesh_fname, info=False):
    ...
    k1 = utility_mvd.compute_local_k(k1, B)
    ...
    k2 = utility_mvd.compute_local_k(k2, B)
    ...
    print(k2[i])
    print(k1[i] + k2[i])
    print(np.allclose(k1[i] + k2[i], k[i]))

    #print(np.allclose(k1 + k2, k))
    #exit()
    ...
    for k in (k, k1, k2):
        ...
        matrixes_g.append(matrix_g)
        vectors_g.append(boundary_vector_g)
        integral_matrixes.append(integral_matrix)
        integral_vectors.append(b_div)

        inner_matrix_gs.append(inner_matrix_g)
        boundary_matrix_gs.append(boundary_matrix_g)
    
    print(np.allclose(matrixes_g[0], matrixes_g[1] + matrixes_g[2]))
    print(np.allclose(vectors_g[0], vectors_g[1] + vectors_g[2]))
    print(np.allclose(integral_matrixes[0], integral_matrixes[1] + integral_matrixes[2]))
    print(np.allclose(integral_vectors[0], integral_vectors[1] + integral_vectors[2]))

    print(matrixes_g[0][i])
    print((matrixes_g[1] + matrixes_g[2])[i])
    print(matrixes_g[1][i])
    print(matrixes_g[2][i])
    print(np.allclose(matrixes_g[0][i], (matrixes_g[1] + matrixes_g[2])[i]))

    print()

    print(np.allclose(inner_matrix_gs[0], inner_matrix_gs[1] + inner_matrix_gs[2]))
    print(np.allclose(boundary_matrix_gs[0], boundary_matrix_gs[1] + boundary_matrix_gs[2]))
    

    for i in range(boundary_matrix_gs[0].shape[0]):
        if np.allclose(boundary_matrix_gs[0][i], (boundary_matrix_gs[1] + boundary_matrix_gs[2])[i]):
            print(True)
        else:
            print(i)
            print(boundary_matrix_gs[0][i])
            print()
            print(boundary_matrix_gs[1][i])
            print(boundary_matrix_gs[2][i])
            print()
            print(boundary_matrix_gs[1][i] + boundary_matrix_gs[2][i])
    
            exit()

    for i in range(matrixes_g[0].shape[0]):
        if np.allclose(matrixes_g[0][i], (matrixes_g[1] + matrixes_g[2])[i]):
            print(True)
        else:
            print(i)
            print(matrixes_g[0][i])
            ...
            print(matrixes_g[1][i])
            print(matrixes_g[2][i])
            # print(np.allclose(inner_matrix_gs[0][i], (inner_matrix_gs[1] + inner_matrix_gs[2])[i]))
            # print(np.allclose(boundary_matrix_gs[0][i], (boundary_matrix_gs[1] + boundary_matrix_gs[2])[i]))

            print(inner)
            print(np.nonzero(inner)[0])
            print(i in np.nonzero(inner)[0])
            break
    ...
    row_div, col_div, data_div = utility_mvd.assemble_coo_sparse_format(quadrangles, integral_matrix)
    row_ru, col_ru, data_ru = utility_mvd.assemble_ru(r, cell_areas)
    row_dudt, col_dudt, data_dudt = utility_mvd.assemble_ru(1/tau, cell_areas)

    # row = np.concatenate((row, row_r))
    # col = np.concatenate((col, col_r))
    # data = np.concatenate((data, data_r))

    A_div_coo = coo_array((data_div, (row_div, col_div)), shape=(groups[4], groups[4]))
    A_div_coo.resize((groups[3], groups[3]))
    A_div_coo.sum_duplicates()
    A_div_coo.eliminate_zeros()
    A_div = A_div_coo.tocsr()

    mask_D_row = (A_div_coo.row < groups[1]) | ((A_div_coo.row >= groups[2]) & (A_div_coo.row < groups[3]))
    mask_D_col = (A_div_coo.col < groups[1]) | ((A_div_coo.col >= groups[2]) & (A_div_coo.col < groups[3]))
    mask_V_row = ((A_div_coo.row >= groups[1]) & (A_div_coo.row < groups[2])) | (A_div_coo.row >= groups[3])
    mask_V_col = ((A_div_coo.col >= groups[1]) & (A_div_coo.col < groups[2])) | (A_div_coo.col >= groups[3])

    mask_DD_VV = (mask_D_row & mask_D_col) | (mask_V_row & mask_V_col)
    mask_DV_VD = (mask_D_row & mask_V_col) | (mask_V_row & mask_D_col)
    
    A_divn = coo_array((A_div_coo.data[mask_DV_VD], (A_div_coo.row[mask_DV_VD], A_div_coo.col[mask_DV_VD])), shape=(groups[3], groups[3])).tocsr()
    A_divn1 = coo_array((A_div_coo.data[mask_DD_VV], (A_div_coo.row[mask_DD_VV], A_div_coo.col[mask_DD_VV])), shape=(groups[3], groups[3])).tocsr()

    A_ru = coo_array((data_ru, (row_ru, col_ru)), shape=(groups[3], groups[3]))
    A_ru.eliminate_zeros()
    A_ru = A_ru.tocsr()

    A_dudt = coo_array((data_dudt, (row_dudt, col_dudt)), shape=(groups[3], groups[3]))
    A_dudt.eliminate_zeros()
    A_dudt = A_dudt.tocsr()

    # Разделил А для декаплинга, а как разделить правую часть?

    # A = A_dudt + sigma*(A_div + A_ru)
    # An = (1 - sigma)*(A_div + A_ru)

    A = A_dudt + sigma*(A_divn1 + A_ru)
    An = A_divn + (1 - sigma)*(A_divn1 + A_ru)
