...
def assemble_const_u(const, cell_areas):
    row = np.arange(cell_areas.size)
    data = const * cell_areas
    return row, row, data
...
def assemble_inner_matrix_g(k, lD, lV):
    inner_matrix_g = k.copy()
    inner_matrix_g[:, :, 0] /= lD[:, np.newaxis]
    inner_matrix_g[:, :, 1] /= lV[:, np.newaxis]
    inner_matrix_g = np.tile(inner_matrix_g, 2)
    inner_matrix_g[:, :, 2:] *= -1
    return inner_matrix_g
...
def assemble_boundary_matrix_g(k, lD, lV, c3, bc3):
    ...
    boundary_matrix_g[:, 1, 0::2] -= (gamma * lV*kVD/kVV)[:, np.newaxis] * boundary_matrix_g[:, 0, 0::2]
...
def assemble_matrix_g(k, lD, lV, c3, bc3, inner, boundary):
    inner_matrix_g = assemble_inner_matrix_g(k[inner], lD[inner], lV[inner])
    boundary_matrix_g = assemble_boundary_matrix_g(k[boundary], lD[boundary], lV[boundary], c3, bc3)
    matrix_g = join_matrix_g(inner_matrix_g, boundary_matrix_g, inner, boundary)
    return matrix_g


def assemble_integral_matrix_without_boundary(matrix_g, lD, lV):
...
def assemble_integral_matrix(matrix_g, lD, lV, boundary, c0, c2, bc0, bc2):
    integral_matrix = assemble_integral_matrix_without_boundary(matrix_g, lD, lV)
    integral_matrix = add_boundary_integrals_to_matrix(integral_matrix, lD, boundary, c0, c2, bc0, bc2)
