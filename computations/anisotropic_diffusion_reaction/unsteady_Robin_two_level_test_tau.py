...
def solve(k, r, f_f, c, bc_f, u_f, q_f, ts, sigma, quadrangle_mesh_fname, info=False):
    ...
    data *= tau
    row_r, col_r, data_r = utility_mvd.assemble_ru(tau*r, cell_areas)
    #row_t, col_t, data_t, b_t = utility_mvd.assemble_du_dt(tau, yn, cell_areas)
    row_t, col_t, data_t = utility_mvd.assemble_ru(1, cell_areas)
    ...
    for i, t in enumerate(ts[1:], 1):
        ...
        b = utility_mvd.assemble_vector_b(boundary_integral_vector, quadrangles, boundary, groups[3])

        # производная по времени
        b_t = cell_areas * yn

        b *= tau
        b += f * cell_areas * tau + b_t
