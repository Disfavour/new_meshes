...
def create_local_coordinate_systems(k, quadrangles, coords, inner, boundary, bc, c):
    ...
    kb = ks[boundary]
    e_D_lenghts = e_D_lenghts.reshape(-1)
    ...
    q_coeffs_boundary[:, 0, 0] = -kb[:, 0, 0] / e_D_lenghts[boundary] + kb[:, 0, 1]*kb[:, 1, 0]/(kb[:, 1, 1]*e_D_lenghts[boundary])
    q_coeffs_boundary[:, 0, 1] = -q_coeffs_boundary[:, 0, 0]
    q_coeffs_boundary[:, 0] -= (c*kb[:, 0, 1]/(2*kb[:, 1, 1])).reshape(-1, 1)
    q_coeffs_boundary[:, 1] = -c / 2

    q_b = np.zeros((quadrangles[boundary].shape[0], 2))
    q_b[:, 0] = kb[:, 0, 1]*bc3 / kb[:, 1, 1]
    q_b[:, 1] = bc3

    e_D_lenghts = e_D_lenghts.reshape(-1, 1)

    integral_kefs = np.zeros((quadrangles[boundary].shape[0], 3, 2))
    integral_kefs[:, 0] = q_coeffs_boundary[:, 0] * e_V_lenghts[boundary]
    integral_kefs[:, 1] = q_coeffs_boundary[:, 1] * e_D_lenghts[boundary]
    integral_kefs[:, 2] = -integral_kefs[:, 0]
    ...
    row_boundary = np.repeat(quadrangles[boundary, :3], 2)
    col_boundary = np.tile(quadrangles[boundary, ::2], 3)
    data_boundary = integral_kefs.copy()
    ...
    b[quadrangles[boundary, 0]] += integral_b[:, 0]
    b[quadrangles[boundary, 2]] += integral_b[:, 1]

    row_boundary1 = quadrangles[boundary, ::2]
    col_boundary1 = quadrangles[boundary, ::2]
    data_boundary1 = integral_kefs

    row = np.concatenate((row_inner, row_boundary, row_boundary1.flat))
    col = np.concatenate((col_inner.flat, col_boundary.flat, col_boundary1.flat))
    data = np.concatenate((data_inner.flat, data_boundary.flat, data_boundary1.flat))
...
if __name__ == '__main__':
    ...
    res = calculate('meshes/rectangle/rectangle_7_quadrangle', k, 1, info=True, plot=False)
    print(*res)
