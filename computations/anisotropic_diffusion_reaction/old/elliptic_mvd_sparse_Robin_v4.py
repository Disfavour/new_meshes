...
def create_local_coordinate_systems(k, quadrangles, coords, inner, boundary, bc, c):
    ...
    q_coeffs_boundary[:, 0] -= (c*kDV/(2*kVV)).reshape(-1, 1)
    ...
    integral_coeffs_boundary = np.zeros((quadrangles[boundary].shape[0], 3, 2))
    ...
    integral_coeffs_boundary[:, 0::2] += (integral_coeffs_boundary[:, 1] / 2).reshape(-1, 1, 2)
...
def calculate(quadrangle_mesh_name, k, c, info=False, plot=False):
    ...
    # np.savetxt('A1_Neumann.txt', A.toarray(), fmt='%5.2f')
