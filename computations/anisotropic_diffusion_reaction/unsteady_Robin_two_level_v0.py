...
def solve(k_f, r_f, f_f, c_f, bc_f, u_f, q_f, ts, sigma, quadrangle_mesh_fname, info=False):
    ...
    global_k = np.transpose(np.atleast_3d(k_f(*quadrangle_centers.T)), axes=(2, 0, 1))

    r = r_f(*nodes[:groups[3]].T)
    ...
    # отделить асембл вектора от матрицы. вроде можно, я же из кефов делаю
    ...
    def collect_data(i):
        ...
        g = utility_mvd.compute_global_g(B, matrix_g, y, quadrangles)
    ...
    for i, t in enumerate(ts[1:], 1):
        ...
        col = np.concatenate((col, col_r, col_t))
