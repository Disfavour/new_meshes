...
def solve(k, r, f_f, c, bc_f, u_f, q_f, ts, sigma, quadrangle_mesh_fname, info=False):
    ...
    matrix_div = coo_array((data_div, (row_div, col_div)), shape=(groups[4], groups[4]))
    matrix_div.resize((groups[3], groups[3]))
    matrix_div.sum_duplicates()
    matrix_div.eliminate_zeros()
    #A_div = A_div_coo.tocsr()

    mask_D_row = (matrix_div.row < groups[1]) | ((matrix_div.row >= groups[2]) & (matrix_div.row < groups[3]))
    mask_D_col = (matrix_div.col < groups[1]) | ((matrix_div.col >= groups[2]) & (matrix_div.col < groups[3]))
    mask_V_row = ((matrix_div.row >= groups[1]) & (matrix_div.row < groups[2])) | (matrix_div.row >= groups[3])
    mask_V_col = ((matrix_div.col >= groups[1]) & (matrix_div.col < groups[2])) | (matrix_div.col >= groups[3])

    mask_DD = mask_D_row & mask_D_col
    mask_DV = mask_D_row & mask_V_col
    mask_VD = mask_V_row & mask_D_col
    mask_VV = mask_V_row & mask_V_col

    indexes_D = np.r_[:groups[1], groups[2]:groups[3]]
    indexes_V = np.r_[groups[1]:groups[2]]
    print(indexes_D.size, np.nonzero(mask_DD)[0].size, groups[1] + groups[3] - groups[2])
    print()
    # Тут неудобно, потому что граничные Делоне после внутренних Вороного и тут надо минусовать офсет

    matrix_div_DD = csr_array((matrix_div.data[mask_DD], (matrix_div.row[mask_DD], matrix_div.col[mask_DD])), shape=(indexes_D.size, indexes_D.size))
    matrix_div_DV = csr_array((matrix_div.data[mask_DV], (matrix_div.row[mask_DV], matrix_div.col[mask_DV])), shape=(indexes_D.size, indexes_V.size))
    matrix_div_VD = csr_array((matrix_div.data[mask_VD], (matrix_div.row[mask_VD], matrix_div.col[mask_VD])), shape=(indexes_V.size, indexes_D.size))
    matrix_div_VV = csr_array((matrix_div.data[mask_VV], (matrix_div.row[mask_VV], matrix_div.col[mask_VV])), shape=(indexes_V.size, indexes_V.size))

    matrix_ru_DD = diags(r * cell_areas[indexes_D], format='csr')
    matrix_ru_VV = diags(r * cell_areas[indexes_V], format='csr')

    matrix_dudt_DD = diags(1/tau * cell_areas[indexes_D], format='csr')
    matrix_dudt_VV = diags(1/tau * cell_areas[indexes_V], format='csr')

    A_DD = matrix_dudt_DD + sigma*(matrix_div_DD + matrix_ru_DD)
    A_VV = matrix_dudt_VV + sigma*(matrix_div_VV + matrix_ru_VV)

    An_DD = -matrix_dudt_DD + (1 - sigma)*(matrix_div_DD + matrix_ru_DD)
    An_VV = -matrix_dudt_VV + (1 - sigma)*(matrix_div_VV + matrix_ru_VV)
    ...
    for i, t in enumerate(ts[1:], 1):
        ...
        #b = sigma*(vector_div + vector_f) + (1 - sigma)*(vector_divn + vector_fn) - (An)@yn

        vector_divs = sigma*vector_div + (1 - sigma)*vector_divn
        vector_fs = sigma*vector_f + (1 - sigma)*vector_fn

        b_D = vector_divs[indexes_D] + vector_fs[indexes_D] - (An_DD + matrix_div_DV)@yn[indexes_D]
        y_D = spsolve(A_DD, b_D)

        b_V = vector_divs[indexes_V] + vector_fs[indexes_V] - (An_VV + matrix_div_VD)@yn[indexes_D]
        y_V = spsolve(A_VV, b_V)

        y = np.concatenate((y_D[:groups[1]], y_V, y_D[groups[1]:]))
...
if __name__ == '__main__':
    ...
    res = solve(*setup_problem(), ts, 1, 'meshes/rectangle/rectangle_7_quadrangle.msh', info=True)
    #print(*res)
