...
def plot2(fname):
    ...
    plt.text(quad_center[0] + 0.00125, quad_center[1] + 0.003, r'$\bm x_m^*$')
    plt.text(quad_center[0] - 0.02, quad_center[1] - 0.01, r'$\Omega_m$')

    plt.text(quad_cell_coords[0, 0] - 0.0075, quad_cell_coords[0, 1] + 0.00, r'$\bm x_i^D$')
    plt.text(quad_cell_coords[2, 0] + 0.0025, quad_cell_coords[2, 1] + 0.00, r'$\bm x_{i+}^D$')

    plt.text(quad_cell_coords[1, 0] + 0.0025, quad_cell_coords[1, 1] - 0.0025, r'$\bm x_j^V$')
    plt.text(quad_cell_coords[3, 0] + 0.0025, quad_cell_coords[3, 1] + 0.00, r'$\bm x_{j+}^V$')

    e1 = e1/55 + quad_center
    plt.text(e1[0] + 0.00, e1[1] - 0.005, r'$\bm e_D$')
    e2 = e2/55 + quad_center
    plt.text(e2[0] + 0.002, e2[1] + 0.00, r'$\bm e_V$')
...
def plot3(quadrangle_mesh, quad, fname):
    plt.rcParams['text.usetex'] = True
    plt.rcParams['text.latex.preamble'] = r'\usepackage{bm}'
    plt.rcParams['font.size'] = 12
    ...
    plt.text(quad_center[0] + 0.01, quad_center[1] - 0.005, r'$\bm x_m^*$')
    plt.text(quad_center[0] - 0.07, quad_center[1] + 0.03, r'$\Omega_m$')

    plt.text(quad_cell_coords[0, 0] + 0.007, quad_cell_coords[0, 1] + 0.00, r'$\bm x_i^D$')
    plt.text(quad_cell_coords[2, 0] + 0.007, quad_cell_coords[2, 1] + 0.00, r'$\bm x_{i+}^D$')

    plt.text(quad_cell_coords[1, 0] + 0.007, quad_cell_coords[1, 1] + 0.00, r'$\bm x_j^V$')
    plt.text(quad_cell_coords[3, 0] - 0.03, quad_cell_coords[3, 1] + 0.00, r'$\bm x_{j+}^V$')

    plt.text(quad_center[0] - 0.01, quad_center[1] + 0.03, r'$\bm e_D$')
    plt.text(quad_center[0] - 0.03, quad_center[1] - 0.03, r'$\bm e_V$')
...
def plot3_bw(quadrangle_mesh, quad, fname):
    ...
    pc_quad = PolyCollection([quad_cell_coords], facecolors=(0, 0, 0, 0.3), edgecolors='k')
    pc_V = PolyCollection([quad_cell_coords[::2]], closed=False, facecolors='none', edgecolors='k')
    pc_D = PolyCollection([quad_cell_coords[1::2]], closed=False, facecolors='none', edgecolors='k')
    ...
    ax.quiver(*quad_center, *e1, color='k', scale=5, width=0.015)
    ax.quiver(*quad_center, *e2, color='k', scale=5, width=0.015)

    ax.plot(quad_cell_coords[::2, 0], quad_cell_coords[::2, 1], 'ok')
    ax.plot(quad_cell_coords[1::2, 0], quad_cell_coords[1::2, 1], 'pk')
    ax.plot(*quad_center, 'sk')
...
if __name__ == '__main__':
    ...
    plot3(f'meshes/ellipse/quadrangle_{1}', 14, f'{folder}/gradient_approximation.pdf')
    plot3_bw(f'meshes/ellipse/quadrangle_{1}', 14, f'{folder}/bw/gradient_approximation.pdf')
