...
for i in range(1, 10):
    ...
    gmsh.open(f'meshes/ellipse/triangle_{i}.msh')
    ...
    #print(rf'{i} & {number_of_D_nodes} & {angles_1.min():.1f} / {angles_1.max():.1f} & {number_of_V_nodes} & {quadrangle_tags.size} \\ \hline')
    print(rf'{i} & {number_of_D_nodes} & {angles_1.min():.1f} / {angles_1.max():.1f} & {number_of_V_nodes} & {quadrangle_tags.size} \\')
