...
def plot_local_basises(quadrangle_mesh, fname):
    ...
    x_i_Dp = node_coords[quadrangle_nodes[32, 2]]
    plt.text(x_i_Dp[0] - 0.03, x_i_Dp[1] - 0.005, r'$\bm x_{i+}^D$')

    x_j_Vp = node_coords[quadrangle_nodes[32, 3]]
    plt.text(x_j_Vp[0] - 0.009, x_j_Vp[1] - 0.017, r'$\bm x_{j+}^V$')

    # (\bm x_m^*)
    plt.text(centers[32, 0] + 0.045, centers[32, 1] - 0.006, r'$\bm e_D$')
    plt.text(centers[32, 0] + 0.000, centers[32, 1] + 0.047, r'$\bm e_V$')
...
if __name__ == '__main__':
    # 26 32
    plot_local_basises(f'meshes/rectangle/rectangle_0_quadrangle', 'images/mvd/grad_approximation.pdf')