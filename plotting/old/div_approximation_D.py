...
def plot_local_basises(quadrangle_mesh, fname):
    ...
    pc_voronoi= PolyCollection([node_coords[cell_nodes[node]]], facecolors=(1, 0, 0, 0.3), edgecolors='none')
    ...
    linked_quads = np.any(quadrangle_nodes == node, axis=1).nonzero()[0]
    ...
    x_i_D = node_coords[node]
    plt.text(x_i_D[0] + 0.01, x_i_D[1] + 0.000, r'$\bm x_i^D$')

    x = node_coords[cell_nodes[node]][0]
    plt.text(x[0] - 0.045, x[1] - 0.02, r'$\bm x_j^V$')

    x = node_coords[cell_nodes[node]][1]
    plt.text(x[0] + 0.003, x[1] + 0.010, r'$\bm x_{j+}^V$')

    x = node_coords[cell_nodes[node]][2]
    plt.text(x[0] + 0.01, x[1] - 0.010, r'$\bm x_{j-}^V$')

    x = node_coords[cell_nodes[node]][3]
    plt.text(x[0] + 0.01, x[1] - 0.000, r'$\bm x_{j\pm}^V$')

    x = node_coords[cell_nodes[node]][4]
    plt.text(x[0] - 0.037, x[1] + 0.017, r'$\bm x_{j\times}^V$')

    x = (node_coords[cell_nodes[node]][0] + node_coords[cell_nodes[node]][1]) / 2
    plt.text(x[0] - 0.000, x[1] + 0.000, r'$\bm x_{m}^*$')

    x = (node_coords[cell_nodes[node]][1] + node_coords[cell_nodes[node]][2]) / 2
    plt.text(x[0] + 0.015, x[1] + 0.000, r'$\bm x_{m+}^*$')

    x = (node_coords[cell_nodes[node]][2] + node_coords[cell_nodes[node]][3]) / 2
    plt.text(x[0] + 0.000, x[1] - 0.035, r'$\bm x_{m-}^*$')

    x = (node_coords[cell_nodes[node]][3] + node_coords[cell_nodes[node]][4]) / 2
    plt.text(x[0] - 0.025, x[1] + 0.015, r'$\bm x_{m\pm}^*$')

    x = (node_coords[cell_nodes[node]][0] + node_coords[cell_nodes[node]][4]) / 2
    plt.text(x[0] - 0.000, x[1] + 0.000, r'$\bm x_{m\times}^*$')
...
if __name__ == '__main__':
    # 6
    plot_local_basises(f'meshes/rectangle/rectangle_0_quadrangle', 'images/mvd/div_approximation_D.pdf')